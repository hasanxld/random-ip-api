...
import { getUserApiKeys, generateApiKey, deleteApiKey } from '../lib/database' // Fixed import path
                  ...
                  <div key={apiKeyData.key} className="border border-gray-200 p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <code className="bg-gray-100 px-3 py-1 text-sm font-mono break-all">
                            {apiKeyData.key}
                          </code>
                          {apiKeyData.isActive ? (
                            ...
                            <span className="bg-red-100 text-red-800 px-2 py-1 text-xs font-medium">
                              Inactive
                            </span>
                          )}
                        </div>
                        <div className="text-sm text-gray-600 space-y-1">
                          <p>
                            <strong>Name:</strong> {apiKeyData.name}
                          </p>
                          <p>
                            <strong>Created:</strong> {new Date(apiKeyData.createdAt).toLocaleDateString()}
                          </p>
                          <p>
                            <strong>Total Requests:</strong> {(apiKeyData.totalRequests || 0).toLocaleString()}
                          </p>
                          {apiKeyData.lastUsed && (
                            <p>
                              <strong>Last Used:</strong> {new Date(apiKeyData.lastUsed).toLocaleString()}
                            </p>
                          )}
                        </div>
                      </div>
                      <button 
                        onClick={() => handleDeleteKey(apiKeyData.key)}
                    ...
                    <div className="bg-gray-50 p-3 text-sm">
                      ...
                      <code className="text-blue-600 break-all">
                        https://randomipapi.netlify.app/api/ip/random?api_key={apiKeyData.key}
                      </code>
                    </div>
                  </div>
                ))}
